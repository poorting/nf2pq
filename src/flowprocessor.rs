    ...
    fn process_flow_fields(&mut self, fields:&BTreeMap<usize, (IPFixField, FieldValue)> , flow: &mut FlowStats) {
        for (_fieldnr, field) in fields.iter() {
            ...
            match field {
                ...
                (IPFixField::ProtocolIdentifier, FieldValue::DataNumber(DataNumber::U8(b))) => { 
                    flow.pr = Some(*b);
                    flow.prs = Some(IP_PROTO_STR[*b as usize].to_string());
                }
    ...
    fn process_v9_flow_fields(&mut self, fields:&BTreeMap<usize, (V9Field, FieldValue)> , flow: &mut FlowStats) {
        for (_fieldnr, field) in fields.iter() {
            ...
            match field {
                ...
                (V9Field::Protocol, FieldValue::ProtocolType(pt) ) => {
                    flow.pr = Some(*pt as u8);
                    flow.prs = Some(IP_PROTO_STR[*pt as usize].to_string());
                },
